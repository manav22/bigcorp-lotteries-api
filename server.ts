...
app.post(
  ...
  async (
    ...
    if (!id) {
      res
        .status(400)
        .json({ error: `Missing required field: ${requiredFieldId}` });
      return;
    }
    ...
    if (!name) {
      res
        .status(400)
        .json({ error: `Missing required field: ${requiredFieldName}` });
      return;
    }
...
app.get(
  ...
  async (
    req: Request,
    res: Response<Lottery[]>,
    res1: Response<APIResponse<Lottery[]>>,
  ): Promise<void> => {
